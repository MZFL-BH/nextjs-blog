            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {categories.map((category) => (
                <Link
                  ...
                  className="group rounded-lg transition-all duration-200 p-6 border hover:shadow-[var(--shadow-medium)] hover:border-[color:var(--borderColor-emphasis)]"
                  style={{
                    ...
                    <div>
                      ...
                      <div className="space-y-1">
                        ...
                        >
                          {(() => {
                            // 计算该分类及其子分类的文章总数
                            let totalArticles = getArticlesByCategory(
                              category.id,
                            ).length;
                            if (category.children) {
                              category.children.forEach((child) => {
                                totalArticles += getArticlesByCategory(
                                  child.id,
                                ).length;
                              });
                            }
                  ...
                  {category.children && (
                    <div className="flex flex-wrap gap-2">
                      {category.children.slice(0, 3).map((child) => (
                        ...
                        >
                          {child.icon}{" "}
                          {locale === "en" ? child.nameEn : child.name}
                        </span>
                      ))}
